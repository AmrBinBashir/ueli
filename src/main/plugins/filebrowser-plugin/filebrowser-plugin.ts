...
export class FileBrowserExecutionPlugin implements ExecutionPlugin, AutoCompletionPlugin, OpenLocationPlugin {
    ...
    private buildSearchResults(filePaths: string[], parentFolder: string, searchTerm?: string): Promise<SearchResultItem[]> {
        return new Promise((resolve, reject) => {
            const unsortedResults = filePaths
                ...
                .map((filePath): SearchResultItem => {
                    return {
                        ...
                        icon: IconManager.Instance.getIcon(Icons.FileIcon),
            ...
            const promises = unsortedResults.map((unsortedResult) => this.fileIconGenerator(unsortedResult.executionArgument, IconManager.Instance.getIcon(Icons.FileIcon), IconManager.Instance.getIcon(Icons.FolderIcon)));
